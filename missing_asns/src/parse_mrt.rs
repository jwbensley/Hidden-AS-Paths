...
/// Extract the route from the RIB entry and the AS path for that route, then add both
pub fn parse_rib_entry(prefix: IpNet, rib_entry: &RibEntry, mrt_data: &RecordData) {
    ...
    let route = Route::new(mrt_data.mrt_fp.clone(), prefix, communities);

    let has_path: bool;
    {
        let paths: std::sync::RwLockReadGuard<'_, Paths> = mrt_data.paths.read().unwrap();
        has_path = paths.has_as_path(&as_sequence);
    }
    if !has_path {
        let mut paths = mrt_data.paths.write().unwrap();
        paths.add(as_sequence, route);
    }
}
